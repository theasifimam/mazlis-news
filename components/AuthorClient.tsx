...
import { MapPin, Calendar, Twitter, Linkedin, Globe, Newspaper, Loader2, UserCheck } from 'lucide-react';
        ...
        return (
            <div className="min-h-screen flex flex-col items-center justify-center bg-background gap-6">
                <div className="text-center">
                    ...
                    <p className="text-zinc-500 text-sm">The requested author profile could not be located.</p>
                </div>
                ...
                <div className="max-w-[1400px] mx-auto px-6 lg:px-12 w-full">
                    ...
                    <div className="flex items-center justify-between pb-4 mb-10 border-b border-zinc-200/60 dark:border-zinc-800/60 text-[10px] font-black uppercase tracking-[0.25em] text-zinc-400">
                        <div className="flex items-center gap-2 text-emerald-500">
                            <UserCheck size={14} />
                            <span className="text-zinc-900 dark:text-white font-bold">AUTHOR PROFILE</span>
                            <span>•</span>
                    ...
                    <div className="flex flex-col md:flex-row items-start gap-8 pb-12 mb-12 border-b border-zinc-200/60 dark:border-zinc-800/60">
                        ...
                        <div className="flex flex-col gap-3 flex-1">
                            ...
                            <div className="flex flex-wrap items-center gap-4 text-zinc-400 text-xs font-bold uppercase tracking-wider mt-1">
                                ...
                                <span className="flex items-center gap-1.5">
                                    <Calendar size={14} /> Joined {user.createdAt ? new Date(user.createdAt).toLocaleDateString('en-US', { month: 'short', year: 'numeric' }) : '2026'}
                                </span>
                            </div>
                            ...
                            {user.socials && (Object.values(user.socials).some(link => link)) && (
                                <div className="flex items-center gap-3 mt-4">
                                    {user.socials.twitter && (
                                        <a href={user.socials.twitter.startsWith('http') ? user.socials.twitter : `https://twitter.com/${user.socials.twitter}`} target="_blank" rel="noopener noreferrer" className="p-2.5 rounded-full border border-zinc-200 dark:border-zinc-800 text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white hover:border-zinc-400 transition-all">
                                            <Twitter size={15} />
                                        </a>
                                    )}
                                    {user.socials.linkedin && (
                                        <a href={user.socials.linkedin.startsWith('http') ? user.socials.linkedin : `https://www.linkedin.com/in/${user.socials.linkedin}`} target="_blank" rel="noopener noreferrer" className="p-2.5 rounded-full border border-zinc-200 dark:border-zinc-800 text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white hover:border-zinc-400 transition-all">
                                            <Linkedin size={15} />
                                        </a>
                                    )}
                                    {user.socials.website && (
                                        <a href={user.socials.website.startsWith('http') ? user.socials.website : `https://${user.socials.website}`} target="_blank" rel="noopener noreferrer" className="p-2.5 rounded-full border border-zinc-200 dark:border-zinc-800 text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white hover:border-zinc-400 transition-all">
                                            <Globe size={15} />
                                        </a>
                                    )}
                                </div>
