...
const clearanceLevels = ['Public Release', 'Tier 1 Subscribers', 'Restricted Research'];
...
export default function NewArticlePage() {
    ...
    const handlePublish = async (e?: React.MouseEvent) => {
        ...
        try {
            ...
            if (response.success) {
                ...
                toast.error(response.error || "Failed to publish article", { id: toastId });
            }
        } catch (error) {
            console.error(error);
    ...
    const handleSaveDraft = async () => {
        ...
        if (!content.replace(/<[^>]*>/g, '').trim()) return toast.error("Content is required to save draft");
        if (selectedTopics.length === 0) return toast.error("At least one topic is required");
        if (!heroFile) return toast.error("Hero image is required");
        ...
        <div className="min-h-screen bg-zinc-50 dark:bg-[#09090b] flex flex-col transition-colors duration-300">
            ...
            <header className="h-16 md:h-20 border-b border-zinc-200 dark:border-zinc-900 px-4 md:px-12 flex items-center justify-between sticky top-0 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl z-[120] transition-colors duration-300">
                <div className="flex items-center gap-3 md:gap-8 min-w-0">
                    <Link href="/dashboard" className="w-9 h-9 md:w-10 md:h-10 shrink-0 rounded-full border border-zinc-200 dark:border-zinc-800 flex items-center justify-center hover:bg-zinc-100 dark:hover:bg-zinc-900 transition-all text-zinc-500 hover:text-zinc-900 dark:hover:text-white bg-white dark:bg-transparent shadow-sm dark:shadow-none">
                        <ChevronLeft size={18} />
                    </Link>
                    <div className="flex flex-col min-w-0">
                        <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.4em] text-zinc-500 dark:text-zinc-600">Composer</span>
                        <span className="text-[11px] md:text-sm font-black text-zinc-900 dark:text-white italic tracking-tight truncate max-w-[120px] md:max-w-none">{title || 'NEW SIGNAL...'}</span>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2 md:gap-4">
                    ...
                    >
                        ...
                        <span className="hidden sm:inline">{isPreview ? 'Close Preview' : 'Live Preview'}</span>
                    </Button>
                    <button
                    ...
                    >
                        <Settings2 size={16} className={showMobileSettings ? "text-zinc-900 dark:text-white" : "text-zinc-500"} />
                    </button>
            ...
            <div className="flex-1 flex overflow-hidden relative">
                {/* Editor Area */}
                <main className={`flex-1 overflow-y-auto p-4 sm:p-12 md:p-24 transition-all duration-700 ${isPreview ? 'opacity-0 scale-95 pointer-events-none absolute inset-0' : 'opacity-100 scale-100 relative'}`}>
                    <div className="max-w-4xl mx-auto flex flex-col gap-8 md:gap-12">
                        ...
                        <textarea
                            ...
                            className="w-full bg-transparent text-4xl sm:text-5xl md:text-6xl font-black font-outfit uppercase tracking-tighter text-zinc-900 dark:text-white placeholder:text-zinc-200 dark:placeholder:text-zinc-900 focus:outline-none resize-none min-h-[100px] leading-[0.9]"
                        />
                        ...
                        <div className="flex flex-col gap-3 border-b border-zinc-200 dark:border-zinc-900 pb-6">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[10px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-600">Topics</span>
                                {selectedTopics.length > 0 && (
                                    <span className="ml-auto text-[9px] font-black text-emerald-600 dark:text-emerald-500 uppercase tracking-widest">{selectedTopics.length} Selected</span>
                                )}
                            </div>
                            {topicsLoading ? (
                                <div className="flex items-center gap-2 text-zinc-400">
                                    <div className="w-3 h-3 border-2 border-zinc-300 border-t-zinc-600 rounded-full animate-spin" />
                                    <span className="text-[9px] font-bold uppercase tracking-widest">Loading topics...</span>
                                </div>
                            ) : availableTopics.length === 0 ? (
                                <p className="text-[10px] text-zinc-400 dark:text-zinc-600 italic">No topics found. <Link href="/topics" className="underline hover:text-zinc-700 dark:hover:text-zinc-400">Create one first.</Link></p>
                            ) : (
                                    ...
                                    {availableTopics.map(topic => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`px-4 py-2 border rounded-full text-[9px] font-black uppercase tracking-widest transition-all cursor-pointer ${isSelected
                        ...
                        <div className="w-full">
                            <Editor value={content} onChange={handleContentChange} placeholder="PROSE ENTRY / START WRITING..." />
                        </div>
                    </div>
                </main>
                            ...
                            >
                                <div className="flex items-center justify-between mb-8">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <h3 className="text-[11px] font-black uppercase tracking-[0.4em] text-zinc-500">Signal Parameters</h3>
                                    </div>
                                    <button onClick={() => setShowMobileSettings(false)} className="w-8 h-8 rounded-full bg-zinc-100 dark:bg-zinc-900 flex items-center justify-center text-zinc-500">
                                        <X size={16} />
                                    </button>
                                </div>
                                <div className="flex flex-col gap-8">
                                    <div className="flex flex-col gap-3">
                                        <label className="text-[9px] font-black uppercase tracking-widest text-zinc-400 italic">Clearance Level</label>
                                        <div className="grid grid-cols-1 gap-2">
                                            {clearanceLevels.map(level => (
                                                <button
                                                    key={level}
                                                    onClick={() => setClearanceLevel(level)}
                                                    className={`py-3 px-4 rounded-xl text-[10px] font-bold uppercase tracking-widest text-left border transition-all ${clearanceLevel === level
                                    ...
                                    <div className="flex flex-col gap-4 border-t border-zinc-100 dark:border-zinc-900 pt-6">
                                        <div className="flex items-center justify-between">
                                            <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Allow Feedback</span>
                                            <Switch checked={allowFeedback} onCheckedChange={setAllowFeedback} />
                                        </div>
                                        <div className="flex items-center justify-between">
                                            <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Enable Encryption</span>
                                    ...
                                    <div className="flex gap-4 pt-6 mt-4 border-t border-zinc-100 dark:border-zinc-900">
                                        <Button
                                            ...
                                            onClick={() => { handleSaveDraft(); setShowMobileSettings(false); }}
                                        >
                                            Save Draft
                                        </Button>
                                        <Button
                                            className="flex-1 h-12 rounded-xl bg-emerald-500 text-white text-[10px] font-black uppercase"
                                            onClick={() => { handlePublish(); setShowMobileSettings(false); }}
                                        >
                                            Publish
                                        </Button>
                                    </div>
                ...
                <AnimatePresence>
                    {isPreview && (
                        ...
                        >
                            <div className="max-w-[1400px] mx-auto bg-white dark:bg-[#09090b] text-zinc-900 dark:text-white shadow-2xl shadow-black/5 dark:shadow-none rounded-[2rem] md:rounded-[3rem] p-6 md:p-32 min-h-[80vh] border border-zinc-100 dark:border-zinc-900 transition-colors duration-500">
                                <div className="flex items-center gap-4 mb-8 md:mb-20 text-zinc-900 dark:text-white">
                                    ...
                                    <span className="text-[9px] md:text-[11px] font-black uppercase tracking-[0.3em] text-zinc-500 dark:text-zinc-600">Feed Projection</span>
                                </div>
                                ...
                                {selectedTopics.length > 0 && (
                                    <div className="flex flex-wrap gap-2 md:gap-3 mb-6 md:mb-8">
                                        {selectedTopics.map(topicId => {
                                            ...
                                            return (
                                                <Badge key={topicId} variant="secondary" className="px-3 md:px-4 py-1.5 md:py-2 bg-zinc-100 dark:bg-zinc-900 text-zinc-600 dark:text-zinc-400 rounded-full text-[9px] md:text-[10px] font-black uppercase tracking-widest border border-zinc-200 dark:border-transparent transition-colors duration-300">
                                                    {topic?.name || 'Topic'}
                                                </Badge>
                                            );
                                        })}
                                ...
                                <div className={`aspect-[16/9] md:aspect-[21/9] ${heroImage ? 'bg-transparent border-none shadow-xl shadow-black/5 dark:shadow-none' : 'bg-zinc-50 dark:bg-zinc-900 border border-zinc-100 dark:border-transparent'} rounded-[1.5rem] md:rounded-[2.5rem] mb-8 md:mb-20 overflow-hidden relative group transition-colors duration-300`}>
                                    {heroImage ? (
                                        <img src={heroImage} alt="Hero Preview" className="w-full h-full object-cover" />
                                    ) : (
                                        <div className="absolute inset-0 flex items-center justify-center text-zinc-300 dark:text-zinc-800 font-black tracking-widest text-sm md:text-xl uppercase italic">No Visual Loaded</div>
                                    )}
                                </div>

                                <div
                                    className="text-lg md:text-2xl font-light leading-relaxed text-zinc-700 dark:text-zinc-300 max-w-4xl ql-editor"
                ...
                <aside className="w-96 border-l border-zinc-200 dark:border-zinc-900 flex flex-col p-8 gap-10 bg-white/50 dark:bg-zinc-950/50 z-10 hidden lg:flex transition-colors duration-300 backdrop-blur-md">
                    <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-[11px] font-black uppercase tracking-[0.4em] text-zinc-500 dark:text-zinc-400">Signal Parameters</h3>
                    </div>

                    <div className="flex flex-col gap-8">
                        <div className="flex flex-col gap-4">
                            <label className="text-[10px] font-black uppercase tracking-widest text-zinc-500 dark:text-zinc-600 italic">Clearance Level</label>
                        ...
                        <div className="flex flex-col gap-4">
                            <div className="flex items-center justify-between">
                                ...
                                {selectedTopics.length > 0 && (
                                    <span className="text-[9px] font-black text-emerald-600 dark:text-emerald-500 uppercase tracking-widest">{selectedTopics.length} selected</span>
                                )}
                            </div>
                            {topicsLoading ? (
                                <div className="flex items-center gap-2 text-zinc-400">
                                    ...
                                    <span className="text-[9px] font-bold uppercase tracking-widest text-zinc-400">Syncing nodes...</span>
                                </div>
                            ) : availableTopics.length === 0 ? (
                                <p className="text-[10px] text-zinc-400 dark:text-zinc-600 italic">
                                    No topics available. <Link href="/topics" className="underline hover:text-zinc-600">Define a node first.</Link>
                                </p>
                            ) : (
                                <div className="flex flex-wrap gap-2">
                                    {availableTopics.map(topic => {
                        ...
                        <div className="flex flex-col gap-4 border-t border-zinc-200 dark:border-zinc-900 pt-8 transition-colors duration-300">
                            <div className="flex items-center justify-between">
                                <label className="text-[10px] font-black uppercase tracking-widest text-zinc-600 dark:text-zinc-500 cursor-pointer" onClick={() => setAllowFeedback(!allowFeedback)}>Allow Feedback</label>
                                <Switch
                            ...
                            <div className="flex items-center justify-between">
                                <label className="text-[10px] font-black uppercase tracking-widest text-zinc-600 dark:text-zinc-500 cursor-pointer" onClick={() => setEnableEncryption(!enableEncryption)}>Enable Encryption</label>
                                <Switch
                    ...
                    <Card className="mt-auto p-6 rounded-3xl bg-white dark:bg-zinc-900/50 border-zinc-200 dark:border-zinc-800/50 shadow-none flex flex-col gap-4 transition-colors duration-300">
                        <span className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-400 dark:text-zinc-600">Article Stats</span>
                        <div className="flex justify-between items-end">
                            <div className="flex flex-col">
                                <span className="text-[9px] font-bold text-zinc-500 dark:text-zinc-700 uppercase">Words</span>
                                <span className="text-xl font-black font-outfit text-zinc-900 dark:text-white leading-none">{wordCount}</span>
                            </div>
                            <div className="flex flex-col text-right">
                                <span className="text-[9px] font-bold text-zinc-500 dark:text-zinc-700 uppercase">Read Time</span>
                                <span className="text-xl font-black font-outfit text-zinc-900 dark:text-white leading-none">{readTime}M</span>
                            </div>
                        </div>
                    </Card>
                </aside>
