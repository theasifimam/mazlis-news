        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (topicsRes.success && topicsRes.data) {
                    setAvailableTopics((topicsRes.data as any).data || topicsRes.data);
                }

                // Fetch article
                const articleRes = await articlesApi.get(id);
                if (articleRes.success && articleRes.data) {
                    const article = (articleRes.data as any).data || articleRes.data;
                    setTitle(article.title);
                    setSelectedTopics(article.topic.map((t: any) => typeof t === 'string' ? t : (t._id || t)));
