...
import { useGetArticlesQuery } from '@/redux/services/articleApi'; // Need to create this too or use existing fetch

const ROLE_CONFIG: Record<string, any> = {
    admin: { label: "Administrator", color: "text-rose-500", bg: "bg-rose-500/5", ring: "ring-rose-500/20", icon: Shield },
    editor: { label: "Editor", color: "text-blue-500", bg: "bg-blue-500/5", ring: "ring-blue-500/20", icon: FileEdit },
    author: { label: "Author", color: "text-emerald-500", bg: "bg-emerald-500/5", ring: "ring-emerald-500/20", icon: FileText },
    reader: { label: "Reader", color: "text-zinc-500", bg: "bg-zinc-500/5", ring: "ring-zinc-500/20", icon: UserIcon },
};

const STATUS_CONFIG: Record<string, any> = {
    active: { label: "Active", dot: "bg-emerald-500", text: "text-emerald-500", bg: "bg-emerald-500/5" },
    suspended: { label: "Suspended", dot: "bg-rose-500", text: "text-rose-500", bg: "bg-rose-500/5" },
    pending: { label: "Pending", dot: "bg-amber-500", text: "text-amber-500", bg: "bg-amber-500/5" },
};
...
export default function UserProfilePage() {
    ...
    // RTK Query hooks
    const { data: userResponse, isLoading: userLoading, refetch: refetchUser } = useGetUserByIdQuery(id);
    const [updateUser, { isLoading: updateLoading }] = useUpdateUserMutation();
    const [updateRole, { isLoading: roleLoading }] = useUpdateUserRoleMutation();
    const [updateStatus, { isLoading: statusLoading }] = useUpdateUserStatusMutation();
    const [deleteUser, { isLoading: deleteLoading }] = useDeleteUserMutation();
    const [resetPassword, { isLoading: resetLoading }] = useResetUserPasswordMutation();
    ...
    const fetchArticles = useCallback(async () => {
        ...
        try {
            const [articlesRes, draftsRes] = await Promise.all([
                api.get(`/articles?author=${id}&limit=5`),
                api.get(`/articles?author=${id}&status=draft&limit=1`)
            ]);
            
            setRecentArticles(articlesRes.data.data || []);
            const totalViews = (articlesRes.data.data || []).reduce((acc: number, art: any) => acc + (art.readCount || 0), 0);
            setStats({
                articles: articlesRes.data.pagination.totalCount || 0,
                drafts: draftsRes.data.pagination.totalCount || 0,
    ...
    const isActionLoading = updateLoading || roleLoading || statusLoading || deleteLoading || resetLoading;
    ...
    const avatarUrl = user.avatar ? (user.avatar.startsWith('http') ? user.avatar : `${STORAGE_URL}${user.avatar}`) : (user?.profilePicture?.url || null);
            ...
            <div className="sticky top-0 z-50 bg-white/80 dark:bg-black/80 backdrop-blur-xl border-b border-zinc-100 dark:border-zinc-900 px-6 py-4">
                <div className="max-w-7xl mx-auto flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                        <div className={`flex items-center gap-2 px-3 py-1 rounded-full border border-zinc-100 dark:border-zinc-800 ${statusConf.bg}`}>
                            <div className={`w-1.5 h-1.5 rounded-full ${statusConf.dot}`} />
                            <span className={`text-[11px] font-bold uppercase tracking-wider ${statusConf.text}`}>{statusConf.label}</span>
                        </div>
                        <Button 
            ...
            <main className="max-w-7xl mx-auto p-6 md:p-10 space-y-12">
                ...
                <header className="flex flex-col md:flex-row items-center md:items-end gap-8 pb-4">
                    <div className="relative group">
                        <div className="absolute inset-0 bg-gradient-to-tr from-zinc-200 to-transparent dark:from-zinc-800 rounded-[2.5rem] -rotate-3 group-hover:rotate-0 transition-transform duration-500" />
                        <Avatar className="w-44 h-44 rounded-[2.5rem] border-[8px] border-white dark:border-zinc-900 shadow-2xl relative z-10 transition-transform duration-500 group-hover:scale-[1.02] overflow-hidden">
                    ...
                    <div className="flex-1 text-center md:text-left space-y-3 pb-2">
                        <div className="flex flex-wrap items-center justify-center md:justify-start gap-4">
                            <h1 className="text-4xl md:text-5xl font-black tracking-tight dark:text-white leading-none whitespace-nowrap">{user.fullName}</h1>
                            <Badge className={`${roleConf.bg} ${roleConf.color} ${roleConf.ring} ring-1 border-none rounded-full px-4 py-1.5 text-xs font-black uppercase tracking-widest`}>
                                {roleConf.label}
                            </Badge>
                        </div>
                        <div className="flex flex-wrap items-center justify-center md:justify-start gap-x-6 gap-y-2 text-zinc-500 font-medium">
                            <div className="flex items-center gap-2 font-bold"><Mail size={16} /> {user.email}</div>
                            <div className="flex items-center gap-2 uppercase text-[10px] tracking-widest"><MapPin size={16} /> {user.location || "Remote Node"}</div>
                            <div className="flex items-center gap-2"><Globe size={16} className="text-zinc-300" /> <span className="opacity-60">@{user.username}</span></div>
                        </div>
                    </div>
                </header>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                    <StatBox label="Total Articles" value={stats.articles} icon={FileText} trend="+2.4%" />
                    <StatBox label="Active Drafts" value={stats.drafts} icon={FileEdit} />
                    <StatBox label="Cumulative Views" value={stats.totalViews} icon={Eye} />
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
                    ...
                    <div className="lg:col-span-8 space-y-10">
                        <section className="space-y-6">
                            ...
                            <div className="bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-[2rem] p-10 shadow-sm relative overflow-hidden group min-h-[200px]">
                                ...
                                <p className="text-lg md:text-xl text-zinc-700 dark:text-zinc-300 leading-relaxed font-medium relative z-10">
                                    {user.bio || "No biography provided for this user. Their professional narrative remains a draft."}
                                </p>
                                {user.expertise && user.expertise.length > 0 && (
                                    <div className="flex flex-wrap gap-2 mt-10 relative z-10">
                                        {user.expertise.map((exp: string) => (
                                            <span key={exp} className="px-4 py-2 rounded-xl bg-zinc-50 dark:bg-zinc-800 text-[11px] font-black uppercase tracking-wider text-zinc-500 border border-zinc-100 dark:border-zinc-800">
                                                {exp}
                                            </span>
                                        ))}
                                    </div>
                        ...
                        <section className="space-y-6">
                            <div className="flex items-center justify-between">
                                ...
                                <Button variant="link" className="text-zinc-400 hover:text-zinc-900 font-bold transition-all h-auto p-0 text-[10px] uppercase tracking-widest">Browse All</Button>
                            </div>
                                ...
                                {recentArticles.length > 0 ? recentArticles.map((article, i) => (
                                    ...
                                    >
                                        <div className="w-20 h-20 rounded-2xl overflow-hidden bg-zinc-100 dark:bg-zinc-800 shrink-0 shadow-inner">
                                            <img src={article.image} alt="" className="w-full h-full object-cover grayscale-[20%] group-hover:grayscale-0 transition-all duration-700" />
                                        </div>
                                        <div className="flex-1 min-w-0 space-y-1">
                                            ...
                                            <div className="flex items-center gap-4 text-xs font-bold text-zinc-400 uppercase tracking-widest">
                                                ...
                                                <span className="flex items-center gap-1.5"><Eye size={14} /> {article.readCount.toLocaleString()}</span>
                                            </div>
                                        </div>
                        ...
                        <section className="space-y-6">
                            ...
                            <div className="bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-[2rem] p-8 space-y-8 shadow-sm">
                                <div className="space-y-4">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400">Clearance Role</label>
                                    <Select 
                                        value={user.role} 
                                        onValueChange={(val) => setConfirmAction({
                                            type: 'role',
                                            isOpen: true,
                                ...
                                <div className="space-y-4">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400">Joined Lifecycle</label>
                                    <div className="p-4 rounded-2xl bg-zinc-50/50 dark:bg-zinc-950/50 border border-zinc-100 dark:border-zinc-800 space-y-3">
                                        <div className="flex justify-between text-xs font-bold uppercase tracking-wider">
                                            ...
                                            <span className="text-zinc-600 dark:text-zinc-300">{format(new Date(user.createdAt), 'MMM d, yyyy')}</span>
                                        </div>
                                        <div className="flex justify-between text-xs font-bold uppercase tracking-wider">
                                            <span className="text-zinc-400">Engagement</span>
                                            <span className="text-zinc-600 dark:text-zinc-300">{user.lastLogin ? format(new Date(user.lastLogin), 'MMM d, yyyy') : "Null"}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-rose-50/30 dark:bg-rose-950/10 border border-rose-100/50 dark:border-rose-900/20 rounded-[2rem] p-8 space-y-4">
                                <Button 
                                    ...
                                    onClick={() => setConfirmAction({
                                        type: 'suspend',
                                        isOpen: true,
                                        title: user.status === 'active' ? "Suspend System Access?" : "Restore System Access?",
                                        description: user.status === 'active' ? "User will be blocked from all nodes." : "Re-authorizing user access.",
                                        variant: user.status === 'active' ? 'destructive' : 'default'
                                    })}
                                ...
                                >
                                    {user.status === 'active' ? <><Ban size={16} /> Suspend Operations</> : <><CheckCircle2 size={16} /> Activate Protocol</>}
                                </Button>
                                ...
                                <Button 
                                    ...
                                    onClick={() => setConfirmAction({
                                        ...
                                        description: "This will purge all user metadata. There is no recovery sequence.",
                                        variant: 'destructive'
                                    })}
...
function SectionHeader({ title }: { title: string }) {
    return (
        <div className="flex items-center gap-4">
            <h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-zinc-400 whitespace-nowrap">{title}</h2>
            <div className="h-[1px] w-full bg-zinc-100 dark:bg-zinc-900" />
        </div>
    );
}

function StatBox({ label, value, icon: Icon, trend }: any) {
    return (
            ...
            <div className="space-y-4 flex-1">
                ...
                <div>
                    ...
                    <div className="flex items-baseline gap-2 mt-1">
                        <span className="text-3xl font-black font-outfit dark:text-white">{value.toLocaleString()}</span>
                        {trend && <span className="text-emerald-500 text-[10px] font-black">{trend}</span>}
                    </div>
                </div>
            </div>
