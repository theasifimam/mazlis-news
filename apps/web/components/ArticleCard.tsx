...
export default function ArticleCard({ article, variant = 'horizontal' }: ArticleCardProps) {
    ...
    return (
        <div className="flex gap-4 items-center group py-4 relative">
            <Link href={articleLink} className="relative w-24 h-24 shrink-0 rounded-2xl overflow-hidden bg-zinc-100 dark:bg-zinc-900">
                <Image
                    src={getImageUrl(article.imageUrl)}
                    alt={article.title}
                    fill
            ...
            <div className="flex flex-col gap-1.5 flex-1 min-w-0">
                <Link href={articleLink}>
                    <h3 className="text-base font-bold font-outfit leading-snug line-clamp-2 transition-colors group-hover:text-zinc-600 dark:group-hover:text-zinc-400 pr-10">
                        {article.title}
                    </h3>
                </Link>
            ...
            <div className="shrink-0">
                <BookmarkButton articleId={article.id} className="!bg-transparent !text-zinc-400 hover:!text-zinc-900 dark:hover:!text-white !p-2 !border-none !shadow-none" />
            </div>
        </div>
    );
}
