                ...
                <div className="max-w-[1400px] mx-auto px-6 md:px-12 h-14 md:h-16 flex items-center justify-between relative transition-all duration-300">
                    ...
                    <nav className="hidden md:flex items-center gap-8 absolute left-1/2 -translate-x-1/2">
                        {navLinks.map((link) => (
                            <Link 
                                key={link.label} 
                                href={link.href}
                                className={`text-[10px] font-black uppercase tracking-[0.2em] transition-all hover:opacity-70 ${contentColor}`}
                            >
                                {link.label}
                            </Link>
                        ))}
                    ...
                    <div className="flex items-center gap-2 md:gap-6">
                        <div className="hidden md:flex items-center gap-6">
                            {!isInitialized ? (
                                <div className={`w-12 h-3 rounded-full animate-pulse transition-colors ${isScrolled ? 'bg-zinc-100 dark:bg-zinc-800' : 'bg-white/20'}`} />
